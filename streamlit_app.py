...
def extract_city(question: str) -> str:
    """Extract city name from the question if present"""
    # Simple pattern to find city names (can be enhanced with more sophisticated NLP)
    import re
    # Look for patterns like "in London" or "for New York"
    match = re.search(r'(?:in|for|at)\s+([A-Z][a-zA-Z\s-]+(?:City|Town|Village)?)', question, re.IGNORECASE)
    if match:
        return match.group(1).strip()
    return None

def process_question(question: str) -> tuple:
    """Process the question to handle city context and check if it's a weather question"""
    # First check if it's a weather-related question
    if not is_weather_question(question):
        return question, False  # Return the original question and False to indicate it's not a weather question
    
    # Extract city from the question
    city = extract_city(question)
    if city:
        st.session_state.last_city = city
    elif st.session_state.last_city and not any(word in question.lower() for word in ['here', 'current location']):
        # Add the last city context if no city is mentioned in the current question
        return f"{question} in {st.session_state.last_city}", True
        
    return question, True  # Return the (possibly modified) question and True to indicate it's a weather question

def is_weather_question(question: str) -> bool:
    """Check if the question is weather-related using simple keyword matching"""
    weather_keywords = [
        'weather', 'temperature', 'rain', 'snow', 'forecast', 'sunny', 
        'cloudy', 'windy', 'humidity', 'precipitation', 'storm', 'fog',
        'hail', 'sleet', 'drizzle', 'breeze', 'thunder', 'lightning',
        'freez', 'hot', 'cold', 'chill', 'thermometer', 'degrees',
        'celsius', 'fahrenheit', 'meteorolog', 'climate', 'barometer',
        'weather forecast', 'weather report', 'weather condition',
        'weather today', 'weather tomorrow', 'weather this week',
        'weather in', 'temperature in', 'forecast for', 'rain in',
        'snow in', 'sunny in', 'cloudy in', 'windy in', 'humid in'
    ]
            ...
            with [col1, col2, col3][i]:
                if st.button(question, key=f"example_{i}", use_container_width=True):
                    with st.spinner("Getting weather..."):
                        processed_question, is_weather = process_question(question)
                        if not is_weather:
                            st.session_state.weather_response = "I can answer only weather questions. Please ask me about weather conditions, temperature, or forecasts for any location!"
                        else:
                            response = weather_api.ask_weather_question(processed_question)
    ...
    if st.session_state.query_made:
                ...
                for i, suggestion in enumerate(suggestions[:4]):
                    with suggestion_cols[i]:
                        if st.button(suggestion[:25] + ".." if len(suggestion) > 25 else suggestion, 
                            ...
                            with st.spinner("Getting weather..."):
                                processed_question, is_weather = process_question(suggestion)
                                if not is_weather:
                                    st.session_state.weather_response = "I can answer only weather questions. Please ask me about weather conditions, temperature, or forecasts for any location!"
                                else:
                                    response = weather_api.ask_weather_question(processed_question)
        ...
        if search_button and search_query.strip():
            with st.spinner("Getting weather information..."):
                processed_question, is_weather = process_question(search_query.strip())
                if not is_weather:
                    st.session_state.weather_response = "I can answer only weather questions. Please ask me about weather conditions, temperature, or forecasts for any location!"
                else:
                    response = weather_api.ask_weather_question(processed_question)
